...
const SettingsPage = () => {
  ...
  const save = async (endpoint: string, data: any, successMsg = "Settings saved!") => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (endpoint === "/users/update-profile" || endpoint === "/users/profile") {
          updateUser(data);
        }
      }
  ...
  const handleApplyTheme = async (theme: string) => {
    ...
    let effectiveTheme: "light" | "dark" = theme as any;
    if (theme === "system") {
    ...
    // Also persist theme preference to backend if needed
    try {
        await api.put("/users/appearance-settings", { ...appearance, theme }, token || undefined);
    } catch (e) {
  ...
  const strLabel = ["", "Very Weak", "Weak", "Fair", "Strong", "Very Strong"][strength];
  const strColor = ["", "bg-destructive", "bg-orange-500", "bg-yellow-500", "bg-green-500", "bg-emerald-500"][strength];
                          ...
                          <div key={key}>
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                placeholder="••••••••"
                                className="w-full pl-10 pr-12 py-3 bg-muted/30 border border-border/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all font-sans"
                              />
                              <button
                                type="button"
                      ...
                      {passwords.newPassword && (
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            ...
                            <p className={`text-xs font-black ${strength(passwords.newPassword) >= 4 ? "text-green-500" : strength(passwords.newPassword) >= 3 ? "text-yellow-500" : "text-destructive"}`}>{strLabel}</p>
                          </div>
                          <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map(i => (
                              <div key={i} className={`h-1.5 flex-1 rounded-full transition-all ${i <= strength(passwords.newPassword) ? strColor : "bg-muted"}`} />
                            ))}
                          </div>
                        </div>
                      )}
            ...
            {activeTab === "integrations" && (
              <motion.div key="integ" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <SectionCard title="Connected Accounts" description="Manage your linked social and productivity accounts" icon={LinkIcon}>
                  {[
                    { name: "Google", icon: "G", color: "bg-red-500", connected: user?.authProvider === "google" },
                    { name: "GitHub", icon: <Github className="w-4 h-4" />, color: "bg-gray-900", connected: !!profile.github },
                    { name: "LinkedIn", icon: <Linkedin className="w-4 h-4" />, color: "bg-blue-600", connected: !!profile.linkedIn },
                    ...
                    <div key={name} className="flex items-center gap-4 p-5 bg-muted/20 rounded-2xl border border-border/50">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">Sync your {name} profile with Eventra</p>
                      </div>
      ...
      <AnimatePresence>
        {deleteConfirm && (
          <>
            ...
            >
              <div className="bg-card border border-destructive/30 rounded-3xl shadow-2xl p-8 max-w-md w-full font-sans">
                ...
                <h3 className="text-xl font-black text-center text-foreground mb-2 tracking-tighter">Delete Account</h3>
